...
The selected Python must be able to import the installed `agent_quality` package without a checkout-specific `PYTHONPATH`; installation fails otherwise, and `hooks status` detects environments that later become unusable. Codex installation also stops with an error when `config.toml` disables hooks or allows managed hooks only, rather than reporting an inactive setup as installed. Agent Quality can configure Codex hooks but cannot verify Codex's separate trust decision. After installation, run the interactive `codex` CLI, enter `/hooks`, and trust `~/.codex/hooks.json` (or `$CODEX_HOME/hooks.json`); then start a new IDE chat. Each repository still opts in independently with `aq init`; global hooks require both `.agent-quality/config.yaml` and the git-ignored, path-bound `.agent-quality/local/.initialized` marker. Captured data remains local to the participating repository under `.agent-quality/local`.
